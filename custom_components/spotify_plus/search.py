...
class SpotifySearch(RestoreEntity):
    ...
    def __init__(self, data: HomeAssistantSpotifyData, user_id: str, name: str, user_country: str) -> None:
    ...
    async def spotify_search(self, call):
        ...
        search_results = {"playlists": [], "albums": [], "related_artists": [], "tracks": [], "profile": []}
        search_items = {}
        ...
        if search_type == search_artist:
            search_items = await self.hass.async_add_executor_job(self.data.client.search, search_param, 5, 0, 'artist', self._user_country)
            ...
            artist_playlists_task = self.hass.async_add_executor_job(self.data.client.search, main_artist_name, 30, 0, 'playlist', self._user_country)
            artist_albums_task = self.hass.async_add_executor_job(self.data.client.artist_albums, main_artist_id, 'album', self._user_country, 50)
            related_artists_task = self.hass.async_add_executor_job(self.data.client.artist_related_artists, main_artist_id)
            top_tracks_task = self.hass.async_add_executor_job(self.data.client.artist_top_tracks, main_artist_id, self._user_country)
            artist_profile_task = self.hass.async_add_executor_job(self.data.client.artist, main_artist_id)
            artist_follow_task = self.hass.async_add_executor_job(self.data.client.current_user_following_artists, [main_artist_id])
            _LOGGER.debug("Artist %s Profile retrieved", search_param)

            related_artists, top_tracks, artist_albums, artist_playlists, artist_profile, artist_follow = await asyncio.gather(
            ...
            if "tracks" in top_tracks:
                ...
                check_follow = await self.hass.async_add_executor_job(self.data.client.current_user_saved_tracks_contains, uris)
            ...
            if artist_albums["total"] > 0:
                ...
                check_follow = await self.hass.async_add_executor_job(self.data.client.current_user_saved_albums_contains, uris)
            ...
            check_follow = await self.hass.async_add_executor_job(self.data.client.current_user_following_artists, uris)
            ...
            if artist_playlists["playlists"]["total"] > 0:
                search_results["playlists"] = [
                    {   "name": item["name"],
                        "artists": None,
                        "image": item["images"][0]["url"] if len(item["images"]) > 0 else None,
                        "uri": item["uri"],
                        "id": item["id"],
                    ...
                    if search_param.lower() in item["name"].lower() or search_param.lower() in item["description"].lower()

                ]

                search_results["playlists"].sort(key=lambda x: x["owner"].lower() == "spotify", reverse=True)

            followers = artist_profile["followers"]["total"]
            formatted_followers = (f"{followers / 1000000:.1f}M" if followers >= 1000000 else
                                f"{followers / 1000:.1f}k" if followers >= 1000 else 
                                str(followers))
            ...
            search_items = await self.hass.async_add_executor_job(self.data.client.search, search_param, 25, 0, 'track,album,playlist', self._user_country)
