...
class SpotifySongData(RestoreEntity):
    ...
    async def get_song_data(self, call):
        ...
        while retry and retry_count < 2:
            try:
                current_playback = await self.hass.async_add_executor_job(self.data.client.currently_playing)
                retry = False
        ...
        if current_playback is None:
            ...
            self._current_track_isrc = current_playback['item']['external_ids']['isrc'].upper()
            ...
            following_album_task = self.hass.async_add_executor_job(
                    self.data.client.current_user_saved_albums_contains, [self._current_album_id]
            )
            following_track_task = self.hass.async_add_executor_job(
                    self.data.client.current_user_saved_tracks_contains, [self._current_track_uri]
            )
            track_details_task = self.hass.async_add_executor_job(
                    self.data.client.track, self._current_track_uri
            )

            following_artist, following_album, following_track, track_details = await asyncio.gather(
                following_artist_task, following_album_task, following_track_task, track_details_task
            )
            ...
            if current_context_uri is not None:
                self._play_source = current_playback.get('context', {}).get('type', 'unknown')
                if current_playback.get('context', {}).get('type', '') == 'playlist':
                    playlist_id = current_context_uri.replace('spotify:playlist:', '')
                    self._spotify_playlist_follow = await self.hass.async_add_executor_job(self.data.client.playlist_is_following, playlist_id, [self._id])
                    spotify_playlist = await self.hass.async_add_executor_job(lambda: self.data.client.playlist(playlist_id, fields='name, images, description'))
            ...
            if self._mm_api_token is not None:
                ...
                if 'message' in response_json and 'body' in response_json['message'] and 'track' in response_json['message']['body']:
                    self._lyrics_link = response_json['message']['body']['track']['track_share_url']
                else:
                    self._lyrics_link = None
            ...
            self._album_tracks = [{'trackNumber':track['track_number'], 'name':track['name'], 'artist':track['artists'][0]['name'], 'uri':track['uri']} for track in album_tracks['items']]

            self._extra_attributes = {"spotify": {
                ...
                "spotify_danceability": round(self._audio_features['danceability'] * 100),
                ...
                "spotify_instrumentalness": round(self._audio_features['instrumentalness'] * 100),
                "spotify_speechiness": round(self._audio_features['speechiness'] * 100),
                "spotify_tempo": self._audio_features['tempo'],
                "spotify_time_signature": self._audio_features['time_signature'],
                "spotify_acousticness": round(self._audio_features['acousticness'] * 100),
    ...
    async def spotify_manage_artist(self, call):
        ...
        if 'artist_id' in call.data and call.data['artist_id']:
            await self.hass.async_add_executor_job(self.data.client.user_follow_artists, [call.data['artist_id']])
            _LOGGER.debug("Spotify Artist %s Added", call.data['artist_id'])
        else:
            if self._current_artist_id:
                await self.hass.async_add_executor_job(self.data.client.user_follow_artists, [self._current_artist_id])
    ...
    async def spotify_manage_album(self, call):
        ...
        if 'album_id' in call.data and call.data['album_id']:
            await self.hass.async_add_executor_job(self.data.client.current_user_saved_albums_add, [call.data['album_id']])
            _LOGGER.debug("Spotify Album %s Added", call.data['album_id'])
        else:
            if self._current_album_id:
                await self.hass.async_add_executor_job(self.data.client.current_user_saved_albums_add, [self._current_album_id])
    ...
    async def spotify_manage_track(self, call):
        ...
        if 'track_id' in call.data and call.data['track_id']:
            await self.hass.async_add_executor_job(self.data.client.current_user_saved_tracks_add, [call.data['track_id']])
            _LOGGER.debug("Spotify Track %s Added", call.data['track_id'])
        else:
            if self._current_track_uri:
                await self.hass.async_add_executor_job(self.data.client.current_user_saved_tracks_add, [self._current_track_uri])
    ...
    async def spotify_manage_playlist(self, call):
        ...
        if 'playlist_id' in call.data and call.data['playlist_id']:
            await self.hass.async_add_executor_job(self.data.client.current_user_follow_playlist, call.data['playlist_id'])
            _LOGGER.debug("Spotify Playlist %s Added", call.data['playlist_id'])
        else:
            if self._spotify_context_uri:
                await self.hass.async_add_executor_job(self.data.client.current_user_follow_playlist, self._spotify_context_uri)
